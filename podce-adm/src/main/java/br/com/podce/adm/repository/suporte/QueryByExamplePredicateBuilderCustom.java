...
public class QueryByExamplePredicateBuilderCustom {
		...
		for (SingularAttribute attribute : type.getSingularAttributes()) {
			...
			if (attribute.getJavaType().equals(String.class)) {
				...
				if (exampleAccessor.isIgnoreCaseForPath(currentPath)) {
					expression = cb.lower(new Translate(cb, expression) );
			...
			} else if(attribute.getJavaType().equals(LocalDate.class)) {
				Expression<LocalDate> expression = from.get(attribute);
				...
				switch (exampleAccessor.getStringMatcherForPath(currentPath)) {
					...
					case EXACT:
						...
						break;
					case STARTING:
						predicates.add(cb.greaterThan(expression, (LocalDate) attributeValue));
						break;
					case ENDING:
						predicates.add(cb.lessThanOrEqualTo(expression, (LocalDate) attributeValue));
						break;
					default:
						throw new IllegalArgumentException(
								"Unsupported StringMatcher " + exampleAccessor.getStringMatcherForPath(currentPath));
